...
# --- Template de la Base de Datos Principal ---
...
if uploaded_file:
    ...
    columnas_requeridas = ['CD', 'CODCLI', 'X', 'Y', 'LU', 'MA', 'MI', 'JU', 'VI', 'SA']
    columnas_faltantes = [col for col in columnas_requeridas if col not in df.columns]
    
    if columnas_faltantes:
        st.error(f"⚠️ Error: Al archivo subido le faltan las siguientes columnas obligatorias: {', '.join(columnas_faltantes)}")
        st.warning("Asegúrate de que los encabezados coincidan exactamente con la plantilla base (cuidado con los espacios en blanco).")
    else:
